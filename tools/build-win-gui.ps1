<#
.SYNOPSIS
Builds the win-gui binary using a pre-built cmpl compiler if available, falling back to dotnet tooling.
#>
...
$OutputDir = Join-Path $RepoRoot "build\win-gui"
$ProfileName = "win-gui"

Write-Host "=== Building win-gui ===" -ForegroundColor Cyan
...
if (-not $BuildSucceeded) {
    ...
    & dotnet publish $ProjectFile -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -o $OutputDir
