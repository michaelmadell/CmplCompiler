<#
.SYNOPSIS
Builds the win-cli binary using a pre-built cmpl compiler if available, falling back to dotnet tooling.
#>
...
$OutputDir = Join-Path $RepoRoot "build\win-cli"
$ProfileName = "win-cli"

Write-Host "=== Building win-cli ===" -ForegroundColor Cyan
...
if (-not $BuildSucceeded) {
    ...
    & dotnet publish $ProjectFile -c Release -r win-x64 -p:IncludeGui=false --self-contained true -p:PublishSingleFile=true -o $OutputDir
