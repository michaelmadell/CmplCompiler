...
set "OUTPUT_DIR=%REPO_ROOT%\build\linux-cli"
set "PROFILE=linux-cli"
...
if !BUILD_SUCCEEDED! neq 1 (
    ...
    dotnet publish "%PROJECT_FILE%" -c Release -r linux-x64 --self-contained true -p:PublishSingleFile=true -o "%OUTPUT_DIR%"
    if !ERRORLEVEL! neq 0 (
